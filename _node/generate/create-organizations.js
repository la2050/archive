      ...
      projects.forEach(project => {
        ...
        for (let prop in project) {
          if (project.hasOwnProperty(prop)) {
            // let compare = subCompare(data.organization_name, project[prop], similarity)
            // if (compare.found === 1) {
            //   isSimilar = true
            // }
            if (project[prop] && typeof(project[prop]) === "string" &&
                getStringForComparison(project[prop]).includes(getStringForComparison(data.organization_name))) {
              isSimilar = true
            }
          }
        }
